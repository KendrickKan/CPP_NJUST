...
struct SubSet
{
    vector<string> sSet; //需要将其升序排序，以方便比对是否存在
};
vector<SubSet> SubSets;//子集集合
...
bool CreateNFA()
{
    ...
    if (fs.is_open())
    {
        ...
        while (getline(fs, line))
        {
            ...
            {
                kNFA.NFAMove[ch1 - 0][strR[0] - 0] = strR[1]; // ch1-'0'代表他的十进制数
            }
            else
            {
                kNFA.NFAMove[ch1 - 0][strR[0] - 0] = "Z"; //终态既包含了直接到字母或者数字的那种，又包含了空
            }
        }
        fs.close();
        return true;
...
void ShowNFA()
{
    for (int i = 0; i < 200; i++)
    {
        for (int j = 0; j < 200; j++)
        {
            if (kNFA.NFAMove[i][j].size() > 0)
                cout << i << " " << j << " " << kNFA.NFAMove[i][j] << " ";
        }
        cout << endl;
    }
}
